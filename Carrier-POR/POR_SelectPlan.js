...
if(session_vars.Channel == "POR"){//Is POR
	...
	//**************************************
	var sortedOffers = Object.keys(offers).map(function(key) {//cannot access Object by position so convert reference to array
		return [Number(key), offers[key]];
	});

	if(sortedOffers.length == 1){
		var sorted = sortedOffers[0][1].sort(
		  function(a, b) {
			// compare Term
			if (a.Term < b.Term)
			  return -1;
	...
	//sortedOffers[Term#][Term Name (always use '1')][Offer #].Price | or: sortedOffers[i][1][e].Price | or: sortedOffers[1][1][0].Price
	//sortedOffers.length
	//******************************************************************************************************
	tg.createOfferTemplates = function(e){
		$("#selectPlanRadios").slideUp(400, function() {//animate/hide
			...
			for(i=0;i <= sortedOffers[e-1][1].length-1; i++){
				try{
					$("#offer").clone().attr('id','offer'+i).addClass('col-sm-'+(Math.floor(12/sortedOffers[e-1][1].length))).appendTo("#selectPlanRadios > .row");//round down incase of 5/7 products
					$("#offer"+i+' .selectPlanTitle').text(sortedOffers[e-1][1][i].Term + " - ");
					$("#offer"+i+' input').val(i).attr("id","radio"+sortedOffers[e-1][1][i].Term.replace(/ +/g, "")+sortedOffers[e-1][1][i].PaymentOption.replace(/ +/g, ""));
					$("#offer"+i+' .planFrequency').text(sortedOffers[e-1][1][i].PaymentOption);
					switch (sortedOffers[e-1][1][i].PaymentOption){
						case "Monthly":
					...
					$("#offer"+i+' .planPrice').text(sortedOffers[e-1][1][i].Price);
					$("#offer"+i+' label.form-control').attr("for","radio"+sortedOffers[e-1][1][i].Term.replace(/ +/g, "")+sortedOffers[e-1][1][i].PaymentOption.replace(/ +/g, "")).attr("id", "planPrice"+sortedOffers[e-1][1][i].Term.replace(/ +/g, "")+sortedOffers[e-1][1][i].PaymentOption.replace(/ +/g, ""));
					if(session_vars.IsDealerEval != "Y"){$("#offer"+i+' .sectionContent').addClass("padBottom0 padTop0");}
				}catch(err){}
			}
			if(sortedOffers[e-1][1].length-1 == 0){//only one item so remove block style
				$("#offer0 .sectionWrap").removeClass('block').addClass('inline-block');
			}

			for(i=sortedOffers[e-1][1].length-1;i >= 0 ; i--){
				try{
					if(session_vars.OEMWarrantyLabor == "3" && i == 0 && session_vars.IsDealerEval != "Y"){//set first consumer product to best value
						...
						break;
					}else if(sortedOffers[e-1][1][i].Term.charAt(0) == 3){
			...
			$("#selectPlanRadios").delay(500).slideDown(400, function() {//animate/show
				if(sortedOffers.length > 1){$('#PlanLevelsWrap').show();}//wait till now to show to prevent css reshuffle
			...
			$("#selectPlanRadios input[type=radio]").change(function(){
				...
				$('#SelectedPackageName').val(sortedOffers[$('#PlanLevels input[name=radio-selectTier]:checked').val()-1][1][$(this).val()].ProductIndicator);
				$('#SelectedPackagePrice').val(sortedOffers[$('#PlanLevels input[name=radio-selectTier]:checked').val()-1][1][$(this).val()].Price);
				$('#SelectedPackageCoverage').val(sortedOffers[$('#PlanLevels input[name=radio-selectTier]:checked').val()-1][1][$(this).val()].Term);
				$('#SelectedPackageFrequency').val(sortedOffers[$('#PlanLevels input[name=radio-selectTier]:checked').val()-1][1][$(this).val()].PaymentOption);
				$('#SelectedPackageTier').val(sortedOffers[$('#PlanLevels input[name=radio-selectTier]:checked').val()-1][1][$(this).val()].Tier);
				switch (sortedOffers[$('#PlanLevels input[name=radio-selectTier]:checked').val()-1][1][$(this).val()].PaymentOption){
					case "Monthly":
			...
			if(Number.isInteger(tg.bestPosition)){
	...
	//******************************************************************************************************
	for(i=1;i <= sortedOffers.length; i++){//setup the Tier radio button offers
		try{
			$("#PlanLevels").append('<div class="inline-block nowrap"></div>');
			$("#TierRadios > input").clone().attr('id','radio-selectTier'+i).val(i).appendTo("#PlanLevels > div:nth-child("+i+")");
			$("#TierRadios > label").clone().attr('for','radio-selectTier'+i).text('Plan '+i).appendTo("#PlanLevels > div:nth-child("+i+")");
		}catch(err){}
	}
	...
	//tg.createOfferTemplates(sortedOffers.length);//init to max on load
	

	$('#PlanLevels > div.inline-block:last-child > label').trigger("click");//init to max on load and show items
	
	if($('#SelectedPackageTier').val()){//check if page history already has tier info and select that on page load
		console.log('#SelectedPackageTier has val');
		//$("#PlanLevels input[name=radio-selectTier][value="+$('#SelectedPackageTier').val()+"]").prop('checked', true);
	}
	if($('#SelectedPackageTermPos').val()){//check if page history already has term info and select that on page load
		console.log('#SelectedPackageTermPos has val');
		//$("#selectPlanRadios label.form-control").eq($('#SelectedPackageTermPos').val()).trigger( "click" );
	}else{
		//$("#selectPlanRadios label:last-child").trigger( "click" );
	}
