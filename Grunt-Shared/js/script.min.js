...
+function(t) {
    ...
    s.VERSION = "3.3.5", s.TRANSITION_DURATION = 150, s.prototype.close = function(e) {
        ...
        e.isDefaultPrevented() || (a.removeClass("in"), t.support.transition && a.hasClass("fade") ? a.one("bsTransitionEnd", i).emulateTransitionEnd(s.TRANSITION_DURATION) : i());
    };
    var n = t.fn.alert;
    ...
    i.VERSION = "3.3.5", i.DEFAULTS = {
        ...
        e += "Text", null == o.resetText && s.data("resetText", s[n]()), setTimeout(t.proxy(function() {
            s[n](null == o[e] ? this.options[e] : o[e]), "loadingText" == e ? (this.isLoading = !0, 
            s.addClass(i).attr(i, i)) : this.isLoading && (this.isLoading = !1, s.removeClass(i).removeAttr(i));
        }, this), 0);
    ...
    i.VERSION = "3.3.5", i.TRANSITION_DURATION = 600, i.DEFAULTS = {
        ...
        var d = o[0], h = t.Event("slide.bs.carousel", {
            relatedTarget: d,
            direction: r
        });
        if (this.$element.trigger(h), !h.isDefaultPrevented()) {
            ...
            o[0].offsetWidth, n.addClass(r), o.addClass(r), n.one("bsTransitionEnd", function() {
                o.removeClass([ e, r ].join(" ")).addClass("active"), n.removeClass([ "active", r ].join(" ")), 
                l.sliding = !1, setTimeout(function() {
                    l.$element.trigger(f);
                }, 0);
            }).emulateTransitionEnd(i.TRANSITION_DURATION)) : (n.removeClass("active"), o.addClass("active"), 
            this.sliding = !1, this.$element.trigger(f)), a && this.cycle(), this;
        }
    };
    var s = t.fn.carousel;
    ...
    i.DEFAULTS = {
        ...
        this.$dialog.on("mousedown.dismiss.bs.modal", function() {
            ...
            s.adjustDialog(), n && s.$element[0].offsetWidth, s.$element.addClass("in"), s.enforceFocus();
...
$(document).ready(function() {
    $("body").attr("id", "bodyID").append("<div id='mobileRespond'></div>").append("<div id='responsive'></div>");
 //setup body/responsive
        try {
        if (session_vars.URLParameterShowVars == "true") {
            $("#bodyID").addClass("ShowVars");
        }
    } catch (err) {}
 //add testing var for custom css show/hide
    //setup header
        $("#header").html("");
 //clear header
        $("#M_H_DIV1").prependTo("#header");
 //move header into header location
        $("#brochurePage, #footer").addClass("hideOnNav");
    if ($(".pinned").length) {
        $("#bodyID").addClass("hasPinned");
    }
    //update phone #s on website
        $(".hasHrefPhoneNum").each(function() {
        try {
            var temp;
            if ($(this).hasClass("TelephoneNumberALT")) {
                temp = session_vars.TelephoneNumberALT;
            } else if ($(this).hasClass("TelephoneNumberCustom")) {
                temp = session_vars.TelephoneNumberCustom;
            } else {
                temp = session_vars.TelephoneNumber;
            }
...
/* script to make the cards on the landing page of equal height */ matchTallest = function(selector) {
    var tallest = 0;
    var thisHeight = 0;
    var cards = $(selector);
    cards.each(function() {
...
/**********************************************************/
/* Bowser - a browser detector * https://github.com/ded/bowser */ !function(e, t) {
...
if (bowser) {
    ...
    if (bowser.android) {
        browserName += "android ";
        browserName += "android" + browserVer + " ";
    } else if (bowser.ios) {
        browserName += "ios ";
        browserName += "ios" + browserVer + " ";
    } else if (bowser.msie) {
        var temp2 = browserVer;
        if (temp2 == "9") {
            browserName += "ie9 ltIE10 ltIE11 ltIE12";
        } else if (temp2 == "10") {
            browserName += "ie10 ltIE11 ltIE12";
        } else if (temp2 == "11") {
            browserName += "ie11 ltIE12";
        }
    }
	...
	*/    if (bowser.msie && browserVer < 10 || bowser.chrome && browserVer < 50 || bowser.firefox && browserVer < 45 || bowser.ios && browserVer < 9 || bowser.safari && browserVer < 8 || bowser.android && browserVer < 5) {
        $("html").addClass("browser-error");
    }
}
...
**********************************************************/
    ...
    //** Show/hide nav on scroll
        tg.checkSize = function(e) {
        //width function results vary so use css property to detect instead
        if ($("#mobileRespond").css("background-repeat") == "repeat") {
            $("#bodyID").addClass("isDesktop").removeClass("isMobile");
        } //is Desktop/tablet
         else {
            $("#bodyID").removeClass("isDesktop").addClass("isMobile");
        }
 //is mobile
        };
    hasScrolled = function(e) {
        ...
        // Make sure they scroll more than delta
                if (Math.abs(tg.lastScrollTop - st) <= tg.delta) {
            return;
        }
        if (st > tg.lastScrollTop && st > tg.navbarHeight && $("#bodyID").hasClass("isMobile")) {
            // Scroll Down
            $("#nav").animate({
                top: "-" + tg.navbarHeight + "px"
            }, "fast").hover(function() {
                $("#nav").stop().animate({
                    top: "0px"
                }, "fast");
 ...
 //hide nav
                        if ($(window).scrollTop() > 400 && $("#bodyID").hasClass("isMobile")) {
                $(".pinned").animate({
                    bottom: "0px"
                }, "fast");
            }
 ...
 // run test on initial page load
                $(window).resize(tg.checkSize);
 // run test on resize of the window
                temp.attr("style", "display:block !important");
 //display none returns 0 otherwise
                tg.M_S_fixedCallCTA = temp.outerHeight(true);
        temp.animate({
            bottom: "-" + (tg.M_S_fixedCallCTA - 7) + "px"
        }, "fast").attr("style", "");
 //animate, then clear display
        });
...
**********************************************************/    $(document).ready(function() {
        $(".mobileOnlyCollapsible_Toggle").click(function(e) {
            //section becomes toggleable on mobile only, always shows on desktop
...
**********************************************************/    $(document).ready(function() {
        ...
        $(".tg-flipswitch-wrap input[type=radio]:checked").prop("checked", true).trigger("change");
 //manual trigger incase of browser history
        });
    /*********************************************************
Toggles
**********************************************************/    $(document).ready(function() {
...
**********************************************************/    $(document).ready(function() {
        try {
            $(".modal").on("show.bs.modal", function(e) {
                tg.scrollTop = $(window).scrollTop();
            });
 // find/record scroll position b/4 opening
                        $(".modal").on("hidden.bs.modal", function(e) {
                $(window).scrollTop(tg.scrollTop);
            });
 // reset scroll position on close
                        $.fn.modal.Constructor.DEFAULTS.backdrop = "static";
 //disable modal close on click background
                        $.fn.modal.Constructor.DEFAULTS.keyboard = false;
 //disable modal close on keyboard 'esc' key
                } catch (err) {}
 //incase bootstap js isn't loaded
        });
    /*********************************************************
Left Rail
**********************************************************/    $(document).ready(function() {
        ...
        if (typeof Storage !== void 0) {
            ...
            if (temp) {
                if (inputType.toLowerCase() == "select") {
                    $(itemName).val(temp).change();
                }
 //dropdown choice + run onchange
                                if (inputType.toLowerCase() == "radio") {
                    $("input:radio[name=" + itemName + "]").filter("[value=" + temp + "]").prop("checked", true).trigger("change");
                }
 //radio selected
                                if (inputType.toLowerCase() == "value") {
                    return temp;
                } //simply return value
                 else {
                    $(itemName).val(temp);
 ...
 //default text input
    ...
    tg.LocalStorageSave = function(storeVal, itemName, inputType) {
        if (typeof Storage !== void 0) {
            //browser compatibility
            if (inputType.toLowerCase() == "select") {
                var temp = $(itemName + " option:selected").val();
            }
 //dropdown choice
                        if (inputType.toLowerCase() == "radio") {
                var temp = $("input[name=" + itemName + "]:checked").val();
            }
 //radio selected
                        if (inputType.toLowerCase() == "value") {
                var temp = itemName;
            } //passed in value to store
             else {
                var temp = $(itemName).val();
            }
 //default to text input
                        if (temp && temp != "mm/dd/yyyy") {
                localStorage[storeVal] = temp;
            }
        }
        return false;
    }
...
**********************************************************/;
    $(document).ready(function() {
        $("#navDrop > ul > li > a").click(function(e) {
            if ($(this).parent().hasClass("current") && $("#bodyID").hasClass("isDesktop")) {
                //only close on desktop sizes
                tg.closeAllNav();
 //close main nav re-clicks
                                return false;
            }
    ...
    tg.toggleSubNav = function(e) {
        if (!$(e).next().hasClass("open")) {
            // comes from menu button so show menu
            $("#navDrop .submenu").not(e.parents()).removeClass("open current").slideUp().removeAttr("style");
 //close all sub navs
                        $("#navDrop .contains-sub").not(e.parents()).removeClass("open current");
 //close all sub navs
                        e.parent().addClass("current");
            $("#bodyID").addClass("openNav");
            e.siblings(".submenu").addClass("open").slideDown();
 //show this item
                } else {
            //comes from nav item so show its drop
            $(e).siblings(".submenu").slideUp().removeClass("open current").removeAttr("style");
 //removeAttr is jQuery v1.6x fix for slideUp not working
    ...
    tg.closeAllSubNav = function(e) {
        ...
        if (!e) {
            $(".open", temp).removeClass("open").slideUp().removeAttr("style");
 //removeAttr is jQuery v1.6x fix for slideUp not working
                        $(".current", temp).removeClass("current");
        } else {
            $(".open", temp).not(e.next()).removeClass("open").slideUp().removeAttr("style");
 //removeAttr is jQuery v1.6x fix for slideUp not working
...
(function(a) {
    ...
    if (!(/iPhone|iPad|iPod/.test(navigator.platform) && /OS [1-7]_[0-9_]* like Mac OS X/i.test(b) && b.indexOf("AppleWebKit") > -1)) return;
    var c = a.document;
